...
function addBreaks(
	...
	if (
		maxBreaks === 0 ||
		(preset.tags.breaks.frequency === "low" && wordCount < 50)
	) {
		return { text, count: 0 };
	}
	...
	if (
		preset.tags.breaks.frequency === "medium" ||
		preset.tags.breaks.frequency === "high"
	) {
		const clauseMatches = [...text.matchAll(CLAUSE_BREAKS)];
		const targetBreaks = Math.min(
			maxBreaks,
			preset.tags.breaks.frequency === "high"
				? clauseMatches.length
				: Math.floor(clauseMatches.length / 2)
		);
		...
		for (const { match } of selectedIndices) {
			const breakTime = Math.floor(
				Math.random() * (preset.tags.breaks.max - preset.tags.breaks.min) +
					preset.tags.breaks.min
			);
...
function addStyleTags(
	...
	if (preset.tags.emphasis && preset.tags.emphasis.length > 0) {
		...
		// Limit emphasis tags
		const maxEmphasis = Math.min(
			MAX_STYLE_TAGS - tagCount,
			parentheticals.length
		);
...
export function transformTextForStyle(
	text: string,
	style: VoiceStyle
): TransformResult {
	// Sanitize input
	...
	const { text: textWithBreaks, count: breakCount } = addBreaks(
		cleanText,
		style
	);
	const { text: finalText, count: styleTagCount } = addStyleTags(
		textWithBreaks,
		style
	);
...
export function extractTagsFromText(
	text: string
): Array<{ type: "break" | "style"; content: string; position: number }> {
	const tags: Array<{
		type: "break" | "style";
		content: string;
		position: number;
